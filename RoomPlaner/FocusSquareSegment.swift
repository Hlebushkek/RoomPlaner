...
extension FocusSquare {
    ...
    */
    ...
    class Segment: SCNNode {
        ...
        let shape: SCNPlane // consider using SCNShape
...
//                let path = UIBezierPath(arcCenter: .zero, radius: 0.25, startAngle: 0, endAngle: 180, clockwise: false)
//                shape = SCNShape(path: path, extrusionDepth: 0)
                shape = SCNPlane(width: Segment.thickness, height: Segment.length)
            case .horizontal:
//                let path = UIBezierPath(arcCenter: .zero, radius: 0.25, startAngle: 0, endAngle: 90, clockwise: false)
//                shape = SCNShape(path: path, extrusionDepth: 0)
                shape = SCNPlane(width: Segment.length, height: Segment.thickness)
            }
//            shape.cornerRadius = 8
        ...
        func open() {
            if alignment == .horizontal {
                shape.width = Segment.openLength
            } else {
                shape.height = Segment.openLength
            }
        ...
        func close() {
            ...
            if alignment == .horizontal {
                oldLength = shape.width
                shape.width = Segment.length
            } else {
                oldLength = shape.height
                shape.height = Segment.length
            }
